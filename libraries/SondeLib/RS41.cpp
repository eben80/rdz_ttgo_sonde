...
static bool validPressure = false;              // pressure is valid
...
static float _RS41_waterVaporSaturationPressure(float Tcelsius)
{
   ...
   T = 0 - 0.4931358f + (1.0f + 4.61e-3f) * T - 1.3746454e-5f * T * T + 1.2743214e-8f * T * T * T;

   /* Plug into H+W equation */
   float p = expf(-5800.2206f / T + 1.3914993f + 6.5459673f * logf(T) - 4.8640239e-2f * T + 4.1764768e-5f * T * T - 1.4452093e-8f * T * T * T);
...
float GetRATemp(uint32_t measuredCurrent, uint32_t refMin, uint32_t refMax, float calT, float taylorT[3], float polyT[6])
{
   ...
   float res = calibration.value.refResistorLow + (calibration.value.refResistorHigh - calibration.value.refResistorLow) * current;
   float x = res * calT;

   float Tuncal = 0 + taylorT[0] + taylorT[1] * x + taylorT[2] * x * x;

   /* Apply calibration polynomial */
   float temperature =
       Tuncal + polyT[0] + polyT[1] * Tuncal + polyT[2] * Tuncal * Tuncal + polyT[3] * Tuncal * Tuncal * Tuncal + polyT[4] * Tuncal * Tuncal * Tuncal * Tuncal + polyT[5] * Tuncal * Tuncal * Tuncal * Tuncal * Tuncal;

   return temperature;
}

float GetPres(uint32_t refMin, uint32_t refMax, uint32_t current) {
return current;
}
...
float GetRAHumidity(uint32_t humCurrent, uint32_t humMin, uint32_t humMax, float sensorTemp, float externalTemp)
{
   ...
   float C = calibration.value.refCapLow + (calibration.value.refCapHigh - calibration.value.refCapLow) * current;
   /* Apply calibration */
   ...
   */
   float RH = sum * _RS41_waterVaporSaturationPressure(sensorTemp) / _RS41_waterVaporSaturationPressure(externalTemp);
...
int RS41::decode41(byte *data, int maxlen)
{
	...
	while(p<maxlen) {  /* why 555? */
      ...
      case 'z': // 0x7a is character z - 7A-MEAS temperature and humidity frame
      {
         ...
         validExternalTemperature = subframeReceived[3] && subframeReceived[4] && subframeReceived[5] && subframeReceived[6] && subframeReceived[7];
         ...
         validHumidity = validExternalTemperature && subframeReceived[0x08] && subframeReceived[0x09] && subframeReceived[0x0A] && subframeReceived[0x0B] && subframeReceived[0x0C] && subframeReceived[0x0D] && subframeReceived[0x0E] && subframeReceived[0x0F] && subframeReceived[0x10] && subframeReceived[0x11] && subframeReceived[0x12];

         // if ((pressureMain -  pressureRef1) / (pressureRef2 - pressureRef1) == 0) {
            if (false) {
            validPressure = false;
         }
         else
         {
            validPressure = true;
            sonde.si()->pressure = GetPres(pressureRef1, pressureRef2, pressureMain);
            Serial.printf("Air Pressure = %f\n", sonde.si()->pressure);
         }
